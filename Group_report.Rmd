...
The confusion matrix measure how well our model is performing under its training. For this particular classification principle, the miscalssification rate is somewhat low (~17%), but the false positive rate is high (~30%). This means that real mail is often being flagged as spam. To reduce this, we can increase the classification principle, as we can see below.
...
Even though the misclassification rate went up with the new criteria, it is still preffered over the old criteria. In this particular classification scenario (spam or not spam), classifying "real mail" as spam is much worse than classifying spam as "real mail". From a user point of view, having some spam on one's inbox is better than missing "real mail" that went to the spam box. Here, the key is analysing the false positive rate, which is much smaller for the new criteria of 0.8.
...
The new model has worse results than the logistic regression with significantly higher miscalssification rates for both test and training data. This approach also results in a higher number of false positives and false negatives.
...
In this case, decreasing K to 1 in our model completly overfitted the data. That is, the predicted values according to the model are exaclty the training values. This leads to worse performance using the testing data: higher misclassification, false positives, and false negatives rates. When run against the training data, the misclassification rate was 0%, as expected.
